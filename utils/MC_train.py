...
# Enable Dropout when train
def apply_dropout(m):
    if isinstance(m, nn.Dropout):
        m.train()

class train_MC_utils(object):
                ...
                for batch_idx, (inputs, labels) in enumerate(self.dataloaders[phase]):
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        if phase == 'train':
                            ...
                            all_MC_outputs = []
                            for _ in range(args.num_MC_sampling):
                                MC_outputs = self.model(inputs)
                                all_MC_outputs.append(MC_outputs)
                            outputs = torch.stack(all_MC_outputs, dim=1)
